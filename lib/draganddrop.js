...
angular.module("ngDragDrop",[])
            ...
            return function (scope, element, attrs) {
                ...
                if (angular.isString(attrs.dragHandleClass)) {
                    ...
                    element[0].childNodes.forEach(function(c){
                        if (angular.element(c).hasClass(dragHandleClass)) dragHandles.push(c);
                    });
                    dragHandles.forEach(function(dh){
                        angular.element(dh).bind("mouseenter", function (e) {
                            element.attr("draggable", true);
                        });
                        angular.element(dh).bind("mouseleave", function (e) {
                            element.attr("draggable", false);
                        });
                    });
                    element.attr("draggable", false);
    ...
    .directive("uiOnDrop", [
		...
		var ctrl=false;
		    ...
		    ctrl=e.ctrlKey;
                    $rootScope.$broadcast("ANGULAR_HOVER", element, e, dragChannel);
                ...
                function onDragLeave(e) {
                  dragging--;
                  $rootScope.$broadcast("ANGULAR_HOVER_STOP", element, e, dragChannel);
                  if (dragging == 0) {
                ...
                function onDragEnter(e) {
                    dragging++;
                    $rootScope.$broadcast("ANGULAR_HOVER_START", element, e, dragChannel);
		    ...
		    if (e.preventDefault) {
                    ...
                    scope.$apply(function () {
                        fn(scope, {$data: sendData.data, $event: e, $channel: sendData.channel, $ctrl:ctrl});
                    });
